...
for v in -1 abc 1.5 '' 0 00 07; do
    mkp
    OUTFILE="$SB/r.txt"
    run_script "$H" --apply --projects "$P" --stale-days "$v" > "$OUTFILE" 2>&1
...
run_script "$H" --apply --projects "$P" > "$OUTFILE" 2>&1
a_eq "$?" 0 "exit 0"
...
OUTFILE="$SB/r.txt"
( cd "$SB" && run_script "$H" --apply --projects projects > "$OUTFILE" 2>&1 )
...
echo "[不存在的目錄、字面上的 ~]"
OUTFILE="$SB/r.txt"
( cd "$SB" && run_script "$H" --apply --projects "$SB/nope" --projects '~' > "$OUTFILE" 2>&1 )
a_eq "$?" 0 "exit 0"
a_log "$OUTFILE" "專案目錄不存在：$SB/nope" "log：不存在的目錄"
a_log "$OUTFILE" "專案目錄不存在：~" "log：字面 ~ 不展開"

echo "[測試本身的防呆：--apply 的 --projects 不在沙盒底下就中止]"
# 路徑刻意選不存在的名稱：就算防呆失效，script 也只會記「專案目錄不存在」，不會刪到東西
NX="nonexistent-cdmc-test-$$"
( run_script "$H" --apply --projects "/$NX" > "$SB/g.txt" 2>&1 )
a_eq "$?" 99 "沙盒外的絕對路徑 → 中止（exit 99）"
( run_script "$H" --apply --projects "$SB/ok:/$NX" > "$SB/g.txt" 2>&1 )
a_eq "$?" 99 "冒號分隔的其中一段在沙盒外 → 中止"
( cd / && run_script "$H" --apply --projects "$NX" > "$SB/g.txt" 2>&1 )
a_eq "$?" 99 "相對路徑以 cwd 解析後在沙盒外 → 中止"
a_log "$SB/g.txt" "ABORT: 路徑不在沙盒底下：'/${NX}'" "中止訊息指出以 cwd 解析後的路徑"
a_eq "$([ -d "$SB" ] && echo yes)" yes "中止只結束子 shell，沙盒還在"
...
OUTFILE="$SB/r.txt"
run_script "$H" --apply --projects "$SB/pX*" > "$OUTFILE" 2>&1
a_eq "$?" 0 "exit 0"
...
OUTFILE="$SB/r.txt"
run_script "$H" --apply --projects "$H" > "$OUTFILE" 2>&1
a_eq "$?" 1 "exit 1"
...
for r in "$H/" "$H/." "$SB" "/" "$H/Library" "$H/Library/Application Support"; do
    OUTFILE="$SB/r2.txt"
    run_script "$H" --projects "$r" > "$OUTFILE" 2>&1
...
OUTFILE="$SB/dry.txt"
run_script "$H" --projects "$PR" > "$OUTFILE" 2>&1
...
run_script "$H" --apply --projects "$PR" > "$OUTFILE" 2>&1
a_eq "$?" 0 "apply exit 0"
